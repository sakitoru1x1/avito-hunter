...
class TelegramNotifier:
    ...
    def _post_with_retry(self, url, *, data, files=None, timeout, retries=4, backoff=3):
        """POST с ретраями на Timeout/ConnectionError и на HTTP 429.

        При VPN связь до api.telegram.org бывает флаковой - один-два повтора
        обычно спасают. Плюс теперь ловим 429 Too Many Requests: читаем
        retry_after из тела ответа (Telegram шлёт в parameters.retry_after)
        или заголовка Retry-After, спим указанное время и повторяем.
        Раньше 429 возвращался как "валидный ответ", и вызывающий код тихо
        получал False - уведомления обрывались после первых 2-3 сообщений.
        """
        last_exc = None
        resp = None
        for attempt in range(retries + 1):
            try:
                resp = self.session.post(url, data=data, files=files, timeout=timeout, proxies=self.proxies)
            ...
            if resp.status_code == 429 and attempt < retries:
                ...
                try:
                    body = resp.json()
                    retry_after = int(body.get("parameters", {}).get("retry_after", backoff))
                except Exception:
                    try:
                ...
                logger.warning(f"Telegram 429 rate-limit, ждём {wait}s (попытка {attempt + 1}/{retries})")
                time.sleep(wait)
                continue
            return resp
        return resp
    ...
    def send_photo(self, photo_url=None, caption=None, parse_mode='HTML', photo_bytes=None):
        ...
        try:
            ...
            if photo_bytes:
                ...
                response = self._post_with_retry(url, data=data, files=files, timeout=15)
            else:
