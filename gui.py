...
class ParserApp:
    def __init__(self, root: ctk.CTk) -> None:
        ...
        self._tg_worker = threading.Thread(target=self._tg_worker_loop, daemon=True)
    ...
    def create_widgets(self) -> None:
        ...
        self.tg_send_cb = ctk.CTkCheckBox(row4, text="Слать в TG", variable=self.tg_send_var)
        self.tg_send_cb.pack(side="left", padx=2)
        ...
        captcha_frame = ctk.CTkFrame(settings_scroll, border_width=1)
        captcha_frame.grid(row=_settings_row, column=1, sticky="ew", padx=10, pady=5)
        _settings_row += 1
        captcha_frame.grid_columnconfigure(1, weight=1)
        ctk.CTkLabel(captcha_frame, text="Антикапча", font=ctk.CTkFont(weight="bold")).grid(row=0, column=0, columnspan=3, pady=(5, 0))

        ctk.CTkLabel(captcha_frame, text="Сервис:").grid(row=1, column=0, sticky="e", pady=2, padx=5)
        self.captcha_service_var = tk.StringVar(value="RuCaptcha")
        self.captcha_service_combo = ctk.CTkComboBox(
            captcha_frame, variable=self.captcha_service_var,
            values=["RuCaptcha", "2Captcha"], width=120, state="readonly",
        )
        self.captcha_service_combo.grid(row=1, column=1, padx=5, sticky="w", pady=2)

        ctk.CTkLabel(captcha_frame, text="API Key:").grid(row=2, column=0, sticky="e", pady=2, padx=5)
        self.captcha_api_key_entry = ctk.CTkEntry(captcha_frame, show="*")
        self.captcha_api_key_entry.grid(row=2, column=1, padx=5, pady=2, sticky="ew")

        self.captcha_check_button = ctk.CTkButton(captcha_frame, text="Проверить баланс", width=140, command=self._check_captcha_balance)
        self.captcha_check_button.grid(row=2, column=2, padx=5, pady=2)

        self.captcha_balance_label = ctk.CTkLabel(captcha_frame, text="", text_color="gray")
        self.captcha_balance_label.grid(row=3, column=0, columnspan=3, padx=5, pady=(0, 8))
...
"""
    ...
    def load_settings(self) -> None:
        ...
        svc = s.captcha_service
        self.captcha_service_var.set("2Captcha" if svc == "2captcha" else "RuCaptcha")
        self.captcha_api_key_entry.delete(0, tk.END)
        self.captcha_api_key_entry.insert(0, s.captcha_api_key)
    ...
    def _log_on_main(self, message: str) -> None:
    ...
    def _on_log_link_click(self, event: tk.Event) -> None:
        tb = self.log_text._textbox
    ...
    def _show_toast(self, text: str, x: int | None = None, y: int | None = None, duration_ms: int = 1000) -> None:
    ...
    def set_status(self, text: str, counter: str | None = None) -> None:
    ...
    def _get_tg_proxies_dict(self) -> dict[str, str] | None:
    ...
    def send_tg_status(self, text: str) -> bool:
    ...
    def send_error_telegram(self, error_text: str) -> bool:
        if not self.update_telegram_notifier():
            return False
    ...
    def _check_captcha_balance(self) -> None:
        ...
        if not api_key:
            self.captcha_balance_label.configure(text="❌ Введите API-ключ", text_color="red")
            return
        svc = "2captcha" if self.captcha_service_var.get() == "2Captcha" else "rucaptcha"
        try:
            from captcha_solver import CaptchaSolver
            solver = CaptchaSolver(api_key, service=svc)
            ok, result = solver.check_balance()
            if ok:
                self.captcha_balance_label.configure(text=f"✅ Баланс: {result}", text_color="green")
            else:
                self.captcha_balance_label.configure(text=f"❌ {result}", text_color="red")
        except Exception as e:
            self.captcha_balance_label.configure(text=f"❌ {e}", text_color="red")

    def update_telegram_notifier(self) -> bool:
        token = self.telegram_token_entry.get().strip()
        chat_id = self.telegram_chat_id_entry.get().strip()
        proxies = self._get_tg_proxies_dict()
    ...
    def _fill_profile_form(self, profile: dict) -> None:
    ...
    def on_profile_select(self, event: tk.Event) -> None:
    ...
    def _collect_profile_from_form(self) -> tuple[str, str, dict]:
    ...
    def profile_set_active(self) -> None:
        if self._current_profile_id is None:
            self.profile_status_label.configure(text="❌ Профиль не выбран", text_color="red")
            return
        try:
    ...
    def save_current_search_as_profile(self) -> None:
    ...
    def _apply_active_profile_on_startup(self) -> None:
    ...
    def profile_load_to_search(self) -> None:
    ...
    def _apply_profile_to_search_tab(self, profile: dict) -> None:
        filters = profile.get("filters") or {}
    ...
    # ---------- Парсинг ----------
    def _build_parse_params(self, query: str, min_price: int, max_price: int, city: str | None) -> ParseParams:
    ...
    def run_parser(self, params: ParseParams) -> None:
        try:
            ...
            if self.auto_update:
                if result.rate_limit_restart:
                    ...
                    try:
                        ...
                        pass
                    self.root.after(5000, self.run_auto_parsing)
                else:
                    self.root.after(100, self.schedule_next_auto)

    # ---------- Парсинг элементов ----------
    def _get_ignore_words(self) -> list[str]:
    ...
    def _startup_history_prompt(self) -> None:
    ...
    def _export_history_to_file(self) -> bool:
    ...
    def _load_image_async(self, session: requests.Session, image_url: str, img_label: ctk.CTkLabel, card: ctk.CTkFrame, gen: int) -> None:
    ...
    def _set_image(self, pil_image: Image.Image, img_label: ctk.CTkLabel, gen: int) -> None:
        if gen != self._results_gen:
            return
        try:
    ...
    def _set_image_fallback(self, url: str, img_label: ctk.CTkLabel, card: ctk.CTkFrame, gen: int) -> None:
        if gen != self._results_gen:
            return
        try:
            if not img_label.winfo_exists():
    ...
    def _build_image_session(self) -> requests.Session:
    ...
    def _create_card(self, item: dict, gen: int, session: requests.Session) -> dict:
    ...
    def _refresh_new_banner(self, visible_items: list[dict]) -> None:
    ...
    def _hide_new_banner(self) -> None:
        try:
            self._new_banner_frame.pack_forget()
        except Exception:
            pass

    def _jump_to_next_new(self) -> None:
    ...
    def hard_stop_handler(self) -> None:
    ...
    def _hard_stop_cleanup(self) -> None:
        try:
    ...
    def run_auto_parsing(self) -> None:
        if not self.auto_update:
            return
        prev = getattr(self, "_parser_thread", None)
        if prev is not None and prev.is_alive():
            self.log("Предыдущий цикл ещё не завершился - пропускаем тик")
            return
        query = self.query_entry.get().strip()
        min_price_str = self.min_price_entry.get().strip()
        max_price_str = self.max_price_entry.get().strip()
    ...
    def schedule_next_auto(self) -> None:
        if not self.auto_update:
            return
        try:
    ...
    def on_closing(self) -> None:
        ...
        # Флаг вниз до cleanup: worker-поток, если живой, перестанет
        # трогать драйвер и не будет спамить ошибками про мёртвую сессию.
        self.stop_parsing = True
        self.auto_update = False
