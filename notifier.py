...
class NotificationService:
    ...
    def send_new_items(self, new_items, img_session):
        """Шлёт пачку новых объявлений. img_session — requests.Session с куками Авито."""
        ...
        for item in new_items:
            ...
            if img and img != "Н/Д" and img.startswith("http"):
                photo_bytes = self.fetch_image_bytes(img_session, img)

            if photo_bytes:
                self._notifier.send_photo(caption=header, photo_bytes=photo_bytes)
            else:
                self._notifier.send_message(header)

            if desc_msg:
                self._notifier.send_message(desc_msg)
